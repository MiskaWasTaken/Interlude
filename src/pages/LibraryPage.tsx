...
export default function LibraryPage() {
  const { folders, isScanning, addFolder, removeFolder, scanLibrary, tracks } =
    useLibraryStore();
  ...
  const handleRemoveFolder = async (path: string) => {
    if (
      confirm(
        `Remove "${path}" from library? This will remove all tracks from this folder.`,
      )
    ) {
    ...
    <div className="p-6 pb-28">
        ...
        {folders.length > 0 ? (
          <div className="space-y-2">
            {folders.map((folder) => (
              ...
              >
                <FolderIcon className="w-5 h-5 text-accent-primary shrink-0" />
                <div className="flex-1 min-w-0">
                  ...
                  {folder.last_scanned && (
                    <p className="text-xs text-text-muted">
                      Last scanned:{" "}
                      {new Date(
                        parseInt(folder.last_scanned) * 1000,
                      ).toLocaleDateString()}
                    </p>
                  )}
                </div>
      ...
      <div className="mt-8">
        ...
        <div className="flex flex-wrap gap-2">
          {["FLAC", "WAV", "ALAC", "AIFF", "MP3", "OGG", "OPUS"].map(
            (format) => (
              <span
                key={format}
