...
export type GradientMode = "animated" | "static" | "off";
...
const GRADIENT_MODE_KEY = "hiflac_gradient_mode";
const GRADIENT_INTENSITY_KEY = "hiflac_gradient_intensity";
...
export function GradientProvider({ children }: { children: ReactNode }) {
  ...
  const [gradientMode, setGradientModeState] = useState<GradientMode>(() => {
    const saved = localStorage.getItem(GRADIENT_MODE_KEY);
    return (saved as GradientMode) || "animated";
  });
  const [intensity, setIntensityState] = useState(() => {
    const saved = localStorage.getItem(GRADIENT_INTENSITY_KEY);
    return saved ? parseFloat(saved) : 0.5;
  });

  // Computed property for backward compatibility
  const gradientEnabled = gradientMode !== "off";

  const setGradientMode = useCallback((mode: GradientMode) => {
    setGradientModeState(mode);
    localStorage.setItem(GRADIENT_MODE_KEY, mode);
  }, []);

  const setIntensity = useCallback((value: number) => {
    setIntensityState(value);
    localStorage.setItem(GRADIENT_INTENSITY_KEY, value.toString());
  }, []);

  const setColorsFromImage = useCallback(
